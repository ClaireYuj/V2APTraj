        ...
        if self.args.SR:
            for b in range(len(nei_list_batch)):
                ...
                if element_states.shape[0] != 1:
                    corr = batch_abs_gt[self.args.obs_length - 1, left: right, :2].repeat(element_states.shape[0], 1,
                                                                                          1)  # [N, N, D]
                    ...
                    for i in range(self.args.pass_time):
                        element_states, cn_state = self.Global_interaction[i](corr_index, nei_index, nei_num,
                                                                              element_states, cn_state)
            ...
            cn_global = cn
        mdn_out = self.Laplacian_Decoder.forward(self.x_encoded_dense, self.hidden_state_global, cn_global,
                                                 epoch)  # predicted trajectory
